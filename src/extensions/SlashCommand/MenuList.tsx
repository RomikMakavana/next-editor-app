...
export const MenuList = React.forwardRef((props: MenuListProps, ref) => {
  ...
  return (
    <Surface
      ref={scrollContainer}
      className="text-black max-h-[min(80vh,24rem)] overflow-auto flex-wrap mb-8 p-2 xs:w-72 shadow-[0_3px_10px_rgb(0,0,0,0.2)]"
    >
        ...
        {props.items.map((group, groupIndex: number) => (
          <React.Fragment key={`${group.title}-wrapper`}>
            ...
            {group.commands.map((command: Command, commandIndex: number) => (
              ...
              >
                <span className="border p-3 rounded-lg text-black">
                  <Icon name={command.iconName} />
                </span>
                <span>
                  <p className="text-black">{command.label}</p>
